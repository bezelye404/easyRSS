...
final class ContentBlockerService {
    ...
    private let ruleListIdentifier = "EasyRSSContentBlockerRules-v3"
    ...
    func prepare() async {
        ...
        if let cached = await lookupRuleList(store: store) {
            ...
            self.isReady = true
            AppLogger.shared.log("Pre-compiled content blocker rules (v3) loaded from store cache.", level: .info, category: .system)
            return
        }
            ...
            store.compileContentRuleList(
                ...
                Task { @MainActor in
                    if let error = error {
                        ...
                        self?.isReady = true
                        AppLogger.shared.log("Successfully compiled native WebKit content blocker rules (v3).", level: .info, category: .system)
                    }
                    continuation.resume()
                }
            }
...
private enum ContentBlockerRules {
    ...
    static var rulesJSON: String {
        var rules: [[String: Any]] = [
            ...
            [
                "trigger": [
                    "url-filter": ".*(tmdwebpush|tmd-consent|tmd-kvkk|notification-widget).*"
                ],
                "action": [
                    "type": "block"
                ]
        ...
        rules.append([
            ...
            "action": [
                ...
                "selector": ".advertisement, .ad-banner, .adsbygoogle, .taboola-container, #outbrain, div[id^='google_ads_'], div[id^='div-gpt-ad'], div[id^='ad-banner'], .sponsor-post, .dfp-ad, #ad-slot, ins.adsbygoogle, .ad-wrapper, .ad-container, [class*='ad-banner'], [class*='ad-modal'], [class*='popup-ad'], [id*='popup-ad'], [class*='newsletter-popup'], [class*='sticky-ad'], [id*='sticky-ad'], [class*='floating-ad'], [class*='ad-interstitial'], [id*='ad-interstitial'], .connatix-slot, .teads-inread, [class*='sponsored-post'], [class*='promoted-content'], [id^='taboola-'], [class^='outbrain-'], #project_Fc_Div, #id_d_gravity_master, .tmdconsent-banner, .tmd-consent, .tmd-kvkk, .fancybox-overlay"
            ]
        ])
